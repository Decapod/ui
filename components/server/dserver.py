...
class ImageController(object):
    ...
    @cherrypy.expose
    def index(self, testingMode=True):
        ...
        if method == "GET":
            ...
            return self.take_picture(testingMode)
    ...
    def take_picture(self, testingMode=True):
        """Capture an image, save it to disk, and generate a thumbnail from it.

        If the application is in testing mode, do not use camera. Instead, get
        an image from the local filesytem."""

        path, filename = "testData/capturedImages/", ""

        if testingMode.lower() != "true":
            status = os.system("gphoto2 --capture-image 2>>capture.log")
            if status != 0:
            ...
            name_to_open = files[int(index) % file_count]
        ...
        thumbname = filename[0:filename.rfind(".")] + "_thumb.jpg"
        im.save(thumbname)
